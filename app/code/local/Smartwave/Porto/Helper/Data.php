...
class Smartwave_Porto_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function __construct()
    {
        ...
        if(!$this->isLocalhost() && $this->isAdmin()){
            $code = Mage::getStoreConfig('porto_license/general/purchase_code');
            $ch = curl_init();

            // Set cURL options
            curl_setopt($ch, CURLOPT_URL, "http://www.newsmartwave.net/envato/verify_purchase.php?item=9725864&version=m1&code=$code");
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_USERAGENT, 'PORTO-PURCHASE-VERIFY');

            // Decode returned JSON
            $result = json_decode( curl_exec($ch) , true );
            $this->_checkedPurchaseCode = $result;
            if(!$this->_checkedPurchaseCode)
                Mage::getSingleton('core/session')->addWarning('Please activate the Smartwave Porto Theme!');
        }
    }
